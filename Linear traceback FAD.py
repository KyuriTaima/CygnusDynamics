...
# 1. On filtre le DataFrame pour ne garder que les Groupes A, D et F
target_names = ["Group A", "Group D", "Group F"]
supergroupFAD = objects_data[objects_data['Object_Name'].isin(target_names)].reset_index(drop=True)

# 2. On extrait les noms et on convertit les colonnes en tableaux NumPy d'un coup (.values)
names_FAD = supergroupFAD['Object_Name'].values
X_gc_kpc = supergroupFAD['X_gc_pc'].values
Y_gc_kpc = supergroupFAD['Y_gc_pc'].values
Z_gc_kpc = supergroupFAD['Z_gc_pc'].values
vx_gc_kms = supergroupFAD['vx_gc_kms'].values
vy_gc_kms = supergroupFAD['vy_gc_kms'].values
vz_gc_kms = supergroupFAD['vz_gc_kms'].values
...
for j in range(len(names_FAD)):
    plt.text(X_gc_kpc[j], Y_gc_kpc[j], names_FAD[j], fontsize=10, color='black', weight='bold')
...
plt.xlabel('X (kpc)')
plt.ylabel('Y (kpc)')
plt.title('Linear Traceback of Supergroup FAD (XY Plane)')
plt.legend()
plt.grid()
plt.show()