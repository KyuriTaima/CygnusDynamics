...
\begin{document}
	...
	Une première approximation consisterait à assimiler le bras spiral à un front parfaitement orthogonal à la rotation galactique. Cependant, les bras spiraux de la Voie Lactée présentent un enroulement géométrique caractérisé par leur angle d'inclinaison (\textit{pitch angle}, noté $i$). Pour le Bras Local, les relevés astrométriques de très haute précision (Reid et al. 2019 \cite{reid2019}) estiment cet angle à $i = 11,4^\circ \pm 1,9^\circ$. En conséquence, la coordonnée radiale ($X$) d'un nuage influe fortement sur l'instant où il sera intercepté par le front d'onde.
	
	Dans un repère local $(X,Y)$ co-mobile avec le LSR, où $X$ pointe vers le Centre Galactique et $Y$ suit la rotation, la vitesse relative de propagation de l'onde $W_y$ obéit à l'équation géométrique :
	\begin{equation}
